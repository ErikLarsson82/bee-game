// Grabbed this from god-know-where and linted it myself
...
class Bezier {
  ...
  translate (v, d1, d2) {
    ...
    const o = this.order
    const d = this.points.map((_, i) => (1 - i / o) * d1 + (i / o) * d2)
    return new Bezier(
      this.points.map((p, i) => ({
        x: p.x + v.x * d[i],
        y: p.y + v.y * d[i]
  ...
  scale (d) {
    ...
    [0, 1].forEach(function (t) {
      ...
      const m = sqrt$1(ov.x * ov.x + ov.y * ov.y)
      ov.x /= m
      ov.y /= m
      np[t + 1] = {
        x: p.x + rc * ov.x,
